...
import org.illaydevel.sql4es.QueryState;
import org.illaydevel.sql4es.model.Column;
import org.illaydevel.sql4es.model.Heading;
import org.illaydevel.sql4es.model.expression.BooleanComparison;
import org.illaydevel.sql4es.model.expression.IComparison;
import org.illaydevel.sql4es.model.expression.SimpleComparison;
...
public class HavingParser extends AstVisitor<IComparison, QueryState> {
    ...
    protected IComparison visitExpression(Expression node, QueryState state) {
        if (node instanceof LogicalBinaryExpression) {
            ...
            return new BooleanComparison(left, right, boolExp.getOperator().equals(LogicalBinaryExpression.Operator.AND));
            ...
            if (compareExp.getRight() instanceof Literal) {
                Object value;
                if (compareExp.getRight() instanceof LongLiteral)
                    value = ((LongLiteral) compareExp.getRight()).getValue();
                else if (compareExp.getRight() instanceof BooleanLiteral)
                    value = ((BooleanLiteral) compareExp.getRight()).getValue();
                else if (compareExp.getRight() instanceof DoubleLiteral)
                    value = ((DoubleLiteral) compareExp.getRight()).getValue();
                else if (compareExp.getRight() instanceof StringLiteral)
                    value = ((StringLiteral) compareExp.getRight()).getValue();
                else {
