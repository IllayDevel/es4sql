...
public class UpdateParser extends AstVisitor<List<Object>, QueryState> {
    ...
    public List<Object> parse(Insert insert, QueryState state) throws SQLException {
        ...
        if (insert.getQuery().getQueryBody() == null)
            throw new SQLException("Unable to insert data without any values");
        if (!(insert.getQuery().getQueryBody() instanceof Values))
            throw new SQLException("Unable to insert data from a query, use ... VALUES (...)");
