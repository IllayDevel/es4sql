...
import org.illaydevel.sql4es.model.BasicQueryState;
import org.illaydevel.sql4es.model.Column;
import org.illaydevel.sql4es.model.Column.Operation;
...
import org.illaydevel.sql4es.model.Utils;
import org.illaydevel.sql4es.model.expression.ColumnReference;
import org.illaydevel.sql4es.model.expression.ICalculation;
import org.illaydevel.sql4es.model.expression.IComparison;
import org.illaydevel.sql4es.model.expression.SimpleCalculation;
...
public class QueryParser extends AstVisitor<ParseResult, Object> {
    ...
    protected ParseResult visitQuerySpecification(QuerySpecification node, Object obj) {
        ...
        if (state.isCountDistinct()) {
            for (Column c : state.getHeading().columns())
                if (c.getOp() == Operation.NONE) {
        ...
        // parse ORDER BY
        if (node.getOrderBy() != null && node.getOrderBy().isPresent() && node.getOrderBy().get().getSortItems().size() > 0) {
    ...
    private ParseResult mergeSelectWithSelect(ParseResult top, ParseResult nested) throws SQLException {
        ...
        if (top.getHeading().hasAllCols()) head = nested.getHeading();
            ...
            for (Column col : top.getHeading().columns()) {
                ...
                if (col2 == null)
                    throw new SQLException("Unable to determine column '" + col.getLabel() + "' within nested query");
    ...
    private ParseResult mergeSelectWithAgg(ParseResult top, ParseResult nested) throws SQLException {
        ...
        if (!(top.getQuery() instanceof MatchAllQueryBuilder))
            throw new SQLException("Unable to combine a WHERE clause with a nested query");
        int limit = Math.min(top.getLimit(), nested.getLimit());
        if (limit <= 0) limit = Math.max(top.getLimit(), nested.getLimit());
        List<OrderBy> sorts = nested.getSorts();
        sorts.addAll(top.getSorts());
        ...
        if (top.getHeading().hasAllCols()) head = nested.getHeading();
            ...
            for (Column col : top.getHeading().columns()) {
                ...
                if (col2 == null) col2 = nested.getHeading().getColumnByLabel(col.getAlias());
                if (col2 == null)
                    throw new SQLException("Unable to determine column '" + col + "' within nested query");
    ...
    private ParseResult mergeAggWithSelect(ParseResult top, ParseResult nested) throws SQLException {
        ...
        else if (!(nested.getQuery() instanceof MatchAllQueryBuilder))
            query = QueryBuilders.boolQuery().must(top.getQuery()).must(nested.getQuery());
