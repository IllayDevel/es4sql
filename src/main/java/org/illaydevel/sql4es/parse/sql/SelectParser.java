...
import org.illaydevel.sql4es.ESQueryState;
...
import org.illaydevel.sql4es.model.expression.SingleValue;
...
public class SelectParser extends AstVisitor<Object, QueryState> {
    ...
    public static String visitDereferenceExpression(DereferenceExpression node) {
        if (node.getBase() instanceof Identifier) {
            ...
            return visitDereferenceExpression((DereferenceExpression) node.getBase()) + "." + node.getField().getValue();
    }
    ...
    private static class ArithmeticParser extends AstVisitor<ICalculation, QueryState> {
        ...
        protected ICalculation visitExpression(Expression node, QueryState state) {
            if (node instanceof LongLiteral) {
                ...
                state.addException("Unknown expression '" + node.getClass().getName() + "' encountered in ArithmeticExpression");
            return null;
        }
    }
